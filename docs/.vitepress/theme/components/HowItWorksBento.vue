      ...
      <div class="pipeline-grid">
        ...
        <div class="stats-card">
          ...
          <div class="health-metrics">
            ...
            <div class="metric">
              <span class="metric-value">{{ metrics.avgDuration }}</span>
...
watch(() => feedMetadata.value, (newData) => {
  if (newData) {
    ...
    if (newData.pipeline) {
      ...
      // Performance stats from fetch
      if (newData.pipeline.fetch?.performance_stats) {
        metrics.value.avgDuration = newData.pipeline.fetch.performance_stats
      }
...
// Feed lists
const v2Feeds = computed(() => {
  const feeds = []
  if (feedMetadata.value?.feeds) {
    Object.entries(feedMetadata.value.feeds).forEach(([platform, data]: [string, any]) => {
      feeds.push({
        name: `${platform}.json`,
  ...
  feeds.push({
    name: 'metadata.json',
    url: '/data/feeds/v2/feed_metadata.json',
    timestamp: feedMetadata.value?.generated || null
  })
  return feeds
})
